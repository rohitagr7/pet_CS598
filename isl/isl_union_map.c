...
__isl_give isl_union_set* traverse_union_map_temp( __isl_keep isl_union_map *may_reads, __isl_keep isl_union_map *may_writes, __isl_keep isl_union_map *must_kills, __isl_keep isl_set *arrays, __isl_keep isl_schedule* schedule){

	isl_union_map *schedule_map = isl_schedule_get_map(schedule);
	isl_set_list *schedule_set = isl_union_set_get_set_list(isl_union_map_domain(schedule_map));
	int n = isl_set_list_n_set(schedule_set);
	isl_union_set *initial_task;
	isl_union_set *initial_task_reads;
	isl_union_set *initial_task_writes;
/*
	for(int i=0; i<n; i++){
		isl_set_list *schedule_set_copy = isl_set_list_copy(schedule_set);
		isl_union_map *reads_copy = isl_union_map_copy(may_reads);
		isl_union_map *writes_copy = isl_union_map_copy(may_writes);
		isl_union_set *this_node = isl_union_set_from_set(isl_set_list_get_set(schedule_set_copy, i));
		isl_union_set *this_node_copy = isl_union_set_copy(this_node);

		isl_union_map *read_map = isl_union_map_intersect_domain(reads_copy, this_node_copy);
		isl_union_set *this_node_copy2 = isl_union_set_copy(this_node);
		isl_union_map *write_map = isl_union_map_intersect_domain(writes_copy, this_node_copy2);
		isl_union_set *this_node_copy3 = isl_union_set_copy(this_node);

		isl_union_set *this_reads = isl_union_map_range(read_map);
		isl_union_set *this_writes = isl_union_map_range(write_map);
		
		if(!(isl_union_set_is_empty(this_reads) && isl_union_set_is_empty(this_writes))){
			if(isl_union_set_is_empty(initial_task)){
				isl_union_set_dump(this_node_copy3);
				isl_union_set_dump(isl_union_set_union(initial_task, this_reads));
				//initial_task = isl_union_set_copy(isl_union_set_union(initial_task, this_node_copy));
				//initial_task_reads = isl_union_set_copy(isl_union_set_union(initial_task_reads, this_reads));			
				//initial_task_writes = isl_union_set_copy(isl_union_set_union(initial_task_writes, this_writes));			
				initial_task_reads = isl_union_set_copy(this_reads);
				initial_task_writes = isl_union_set_copy(this_writes);
			}
			else {
				int num_raw = 0;
				int num_waw = 0;
				int num_war = 0;
				isl_set_list *read_list = isl_union_set_get_set_list(this_reads);
				isl_set_list *write_list = isl_union_set_get_set_list(this_writes);
				int n_reads = isl_set_list_n_set(read_list);
				int n_writes = isl_set_list_n_set(write_list);
				for(int j=0; j<n_reads; j++){
					isl_union_set *initial_task_reads_copy = isl_union_set_copy(initial_task_reads);
					isl_union_set *initial_task_writes_copy = isl_union_set_copy(initial_task_writes);
					isl_union_set *read = isl_union_set_from_set(isl_set_list_get_set(read_list, j));
					if(!isl_union_set_is_empty(isl_union_set_intersect(initial_task_writes_copy, read))){
						num_raw++;
					}
				}
				for(int j=0; j<n_writes; j++){
					isl_union_set *initial_task_reads_copy = isl_union_set_copy(initial_task_reads);
					isl_union_set *initial_task_writes_copy = isl_union_set_copy(initial_task_writes);
					isl_union_set *write = isl_union_set_from_set(isl_set_list_get_set(write_list, j));
					if(!isl_union_set_is_empty(isl_union_set_intersect(initial_task_writes_copy, isl_union_set_copy(write)))){
						num_waw++;
					}
					if(!isl_union_set_is_empty(isl_union_set_intersect(initial_task_reads_copy, isl_union_set_copy(write)))){
						num_waw++;
					}
				}
			}
...
*/
	...
	isl_union_map *read_copy = isl_union_map_copy(may_reads);
	isl_union_map *write_copy = isl_union_map_copy(may_writes);
	isl_union_set *read_domain = isl_union_map_domain(may_reads);
	isl_union_set *write_domain = isl_union_map_domain(may_writes);
	isl_union_set *read_range = isl_union_map_range(read_copy);
	isl_union_set *kill_range = isl_union_map_range(must_kills);
	isl_union_set *write_range = isl_union_map_range(isl_union_map_subtract_range(write_copy, kill_range));
	isl_union_set *statements = isl_union_set_union(read_domain, write_domain);
	return statements;
